...
class viewCategory:
    def __init__(self):
        ...
        self.ml=Label(self.root,text="View Category",font=('',36,'bold'))
        ...
        self.style.configure("Treeview Heading",font=('',12,''),rowheight=50)
    ...
    def searchBtn(self):
        ...
        q=f"select * from category1 where name like'%{data}%' "
        self.cr.execute(q)
        result=self.cr.fetchall()
        for row in self.categoryTable.get_children():
            self.categoryTable.delete(row)
    ...
    def deleteBtn(self):
        ...
        if len(row)==0:
            msg.showwarning("Invalid","Please select a row you want to delete")
        else:
            row_id=self.categoryTable.selection()[0]
            item=self.categoryTable.item(row_id)
            data=item['values']
            confirm=msg.askyesno("","Are you sure you want to Delete Category?")
            if confirm :
                q=f"delete from category1 where name='{data[0]}'"
                self.cr.execute(q)
                self.conn.commit()
    ...
    def updateCategory(self):
        if self.txt2.get() == '' and self.txt3.get() == '':
            ...
            q = f"update category1 set description='{description}' where name='{name}'"
            self.cr.execute(q)
            self.conn.commit()
