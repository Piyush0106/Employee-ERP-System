...
class viewemployee:
    ...
    #     bg_color = '#FFFFFF'
    #
    #     # Use the style to configure the Treeview widget's background
    #     style.configure("Treeview", background=bg_color)
    #
    #     self.mainLabel = Label(self.root, text="View Employee", font=("", 32, 'bold'))
    ...
    #
    #     self.employeetable = ttk.Treeview(self.root, column=('employee_id', 'name', 'father_name', 'email', 'mobile', 'address', 'category', 'department', 'image',))
    #     # self.employeetable.heading('employee_id', text="ID")
    #     # self.employeetable.heading('name', text="Name")
    #     # self.employeetable.heading('father_name', text="Father Name")
    ...
    #     # self.employeetable.column('employee_id', width=10, anchor="center")
    #     # self.employeetable.column('name', width=10, anchor="center")
    #     # self.employeetable.column('father_name', width=10, anchor="center")
    #     # self.employeetable.column('email', width=10, anchor="center")
    #     # self.employeetable.column('mobile', width=10, anchor="center")
    #     # self.employeetable.column('address', width=10, anchor="center")
    #     # self.employeetable.column('category', width=10, anchor="center")
    #     # self.employeetable.column('department', width=10, anchor="center")
    #     # self.employeetable.column('image', width=10, anchor="center")
    #     #
    #     # self.employeetable.pack(expand=True, fill='both', padx=20, pady=20)
    ...
    #
    #     # Define colors
    #     header_bg_color = '#ADD8E6'
    #     header_fg_color = 'black'
    #     row_bg_color = 'white'
    #     row_fg_color = 'black'
    #
    #     self.employeetable.config(bg=row_bg_color, fg=row_fg_color, selectbackground="lightblue",
    #                               selectforeground="black", bd=2, font=('Arial', 12))
    #
    ...
    def getemployeeinfo(self):
        q = "select employee_id,name,father_name,email,mobile,address,category,department,image from employee"
        self.cr.execute(q)
    ...
    def openUpdateWindow(self, event):
        ...
        self.txt7 = ttk.Combobox(self.updateForm, font=font, values=self.values())
        ...
        self.txt8 = ttk.Combobox(self.updateForm, font=font, values=self.values1())
        ...
        self.txt9 = Entry(self.updateForm, font=self.font, width=35)
    ...
    def updateemployee(self):
        ...
        if (len(name) == 0 or len(father_name) == 0 or len(email) == 0 or len(mobile) == 0 or len(address) ==0 or
            ...
            msg.showwarning("Warning", "Please enter all fields")
        else:
            if '@' in self.txt4.get() and '.' in self.txt4.get():
                if len(self.txt5.get()) == 10 and self.txt5.get().isdigit():
                    q = (f"update employee set name='{name}', father_name='{father_name}',  mobile='{mobile}',email='{email}',address='{address}',department='{department}',category='{category}',image='{image}' WHERE employee_id='{id}'")
                    self.cr.execute(q)
                    self.conn.commit()
                    msg.showinfo("Success", "Employee has been updated")
                    self.root1.destroy()
                    self.refreshData()
                else:
                    msg.showwarning("Warning", "Please enter valid Mobile Number")
            else:
                msg.showwarning("Warning", "Please enter valid Email ")
    ...
    def searchemployee(self):
        ...
        q = f"select employee_id,name,father_name,email,mobile,address,category,department,image from employee where name like '%{data}%' or address like '%{data}%'"
        self.cr.execute(q)
        result = self.cr.fetchall()
    ...
    def deleteemployee(self):
        row = self.employeetable.selection()
        if len(row) == 0:
            msg.showwarning("Warning", "Please select the Item")
        else:
            row_id = row[0]
            items = self.employeetable.item(row_id)
            data = items['values']

            confirm = msg.askyesno("", "Are you sure you want to Delete ?")
            if confirm:
                q = f"delete from employee where employee_id='{data[0]}'"
                self.cr.execute(q)
                self.conn.commit()
                msg.showinfo("Success", "Employee has been deleted")
                self.refreshData()
            else:
                msg.showwarning("Success", "Deletion Aborted")
    ...
    #
    #         if len(faces) == 0:
    #             msg.showwarning("Warning", " Image is not valid",parent=self.openUpdateWindow())
    #
    #         else:
    #             msg.showinfo("Success", " Image is Valid",parent=self.openUpdateWindow())
    #             img_name = f"{name}.png"
    #             name = f"employeeImage/{name}.png"
    #             cv2.imwrite(name, image)
    #             self.txt9.insert(0, img_name)
    #             # cv2.imwrite(img_name)
    #     else:
    #         msg.showwarning("Warning", " Enter Name",parent=self.openUpdateWindow())
