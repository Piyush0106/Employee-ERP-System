...
class viewhod:
    ...
    #
    #     self.mainLabel=Label(self.root,text="View HOD",font=('',28,'bold'))
    #     self.mainLabel.pack()
    #
    ...
    #     self.hodTable.column('hod_id',width=10,anchor='center')
    #     self.hodTable.column('name', width=10, anchor='center')
    #     self.hodTable.column('email',width=10,anchor='center')
    #     self.hodTable.column('mobile',width=10,anchor='center')
    #     self.hodTable.column('qualifications',width=10,anchor='center''')
    #
    #     self.style=ttk.Style()
    #     self.style.configure("Treeview Heading",font=('',14),rowheight=50)
    #     self.style.configure("Treeview",font=('',14),rowheight=50)
    #
    #
    #
    #     self.hodTable.pack(expand=True,fill='both',padx=20,pady=20)
    #     self.gethodInfo()
    ...
    def __init__(self):
        ...
        self.style.configure("Custom.Treeview.Heading", font=('', 14, 'bold'), background='#4682B4', foreground='white',
                             relief='flat')
    ...
    def updatehod(self):
        ...
        if len(name) == 0 or len(email) == 0 or len(mobile) == 0 or len(qualifications) == 0:
            msg.showerror("Not Working","Need to fill all fields")
        else:
            if mobile.isdigit() and len(mobile)==10 :
                if '@' in email and '.' in email :
                    ...
                    self.cr.execute(q)
                    self.conn.commit()
                    msg.showinfo("Success","HOD Updated")
                    self.root1.destroy()
                    self.refreshData()
                else:
                    msg.showwarning("Not working","Email Format Invalid")
            else:
                msg.showwarning("Not Running ","Invalid Mobile Number")

    def gethodInfo(self):
        q="select hod_id,name,email,mobile,qualifications from hod"
        self.cr.execute(q)
    ...
    def deletehod(self):
        ...
        if len(row)==0:
            ...
            if confirm:
                ...
                self.cr.execute(q)
                self.conn.commit()
                msg.showinfo("Success","Deleted Successfully")
                self.refreshData()
            else:
                msg.showinfo("Success","Deletion Aborted")
